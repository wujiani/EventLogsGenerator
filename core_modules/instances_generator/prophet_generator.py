...
class ProphetGenerator:
    ...
    def _compare_models(self, acc, model_exist, file):
        metadata_file = os.path.join(self.parms['ia_gen_path'],
            self.parms['file'].split('.')[0]+'_prf_meta.json')
    ...
    def _discover_model(self):
        ...
        with open(os.path.join(
                self.temp_output,
                self.parms['file'].split('.')[0]+'_prf.json'), 'w') as fout:
    ...
    def _transform_features(df_train):
        df_train = df_train.groupby('caseid').start_timestamp.min().reset_index()
        ...
        # quantile = df_train.y.quantile(0.9)
        # df_train.loc[df_train.y>quantile, 'y'] = None
    ...
    @staticmethod
    def _generate(num_instances, start_time, ia_gen_path, file):
        model_path = os.path.join(ia_gen_path,
                                  file.split('.')[0]+'_prf.json')
        with open(model_path, 'r') as fin:
            m = model_from_json(json.load(fin))  # Load model
        metadata_file = os.path.join(ia_gen_path,
                                     file.split('.')[0]+'_prf_meta.json')
        # compare with existing model
        with open(metadata_file) as file:
        ...
        while n_gen_inst < num_instances:
            future = pd.date_range(start=start_time,
                                   end=(start_time+timedelta(days=30)),
                                   freq='H').to_frame(name='ds', index=False)
    ...
    def generate(self, num_instances, start_time, n_size=5):
        ...
        """
        self.times = self._generate(num_instances, start_time, self.parms['ia_gen_path'], self.parms['file'])
